...
**核心思路**：输入一句话的产品想法 → 输出一个可直接运行的 React + Vite + Hono 全栈网站，包含前台店铺、购物车、结账、管理后台、AI 导购，以及 EdgeOne Pages 部署配置。
...
├── src/
│   ├── components/
│   │   └── AiConcierge.tsx    # AI 购物导购组件
│   ├── pages/
│   │   ├── HomePage.tsx       # 首页 / 产品展示
│   │   ├── CartPage.tsx       # 购物车页面
│   │   ├── CheckoutPage.tsx   # 结账页面
│   │   ├── AccountPage.tsx    # 用户账户页面
...
编辑 `src/styles.css` 或使用 CSS-in-JS / Tailwind 等方案。
